#!/usr/bin/env python3
"""One-time local setup for the strict read-only Telegram server.
...
def _write_env(values: dict[str, str]) -> None:
    ...
    if output and output[-1] != "":
        output.append("")
    if remaining:
        output.append("# Managed by setup_readonly.py")
...
def main() -> None:
    ...
    _write_env(
        {
            ...
            "TELEGRAM_DEVICE_MODEL": "Telegram MCP Read-only",
            "TELEGRAM_SYSTEM_VERSION": platform.system(),
    ...
    print("No session string was printed or stored in client configuration.")
