...
_EXPOSED_TOOLS_MODES = {"all", "read-only", "strict-read-only"}
_EXPOSED_TOOLS_ALLOW_SEPARATOR = "+"

# This mode is intentionally independent from MCP annotations.  Upstream's
# annotation-based ``read-only`` mode is useful as a convenience, but a single
# incorrect ``readOnlyHint`` is enough to expose a mutating Telegram RPC.  Keep
# this explicit list small and review additions one-by-one.  The sole local
# side-effect is ``download_media_readonly``, which can only write generated
# filenames below TELEGRAM_READONLY_DOWNLOAD_DIR.
STRICT_READ_ONLY_TOOLS = frozenset(
    {
        ...
        "get_user_photos",
        "get_user_status",
        # Chats, folders, topics, and account-wide discovery.
        "get_chats",
        "list_chats",
        "get_chat",
        "get_full_chat",
        ...
        "get_banned_users",
        "get_recent_actions",
        # Media metadata and a path-confined local downloader.
        "get_media_info",
        "get_sticker_sets",
        "get_gif_search",
        "download_media_readonly",
        # Passive incoming-message waits. These only mutate in-process queues.
...
def _apply_exposed_tools_mode(server: FastMCP = mcp, mode: Optional[str] = None) -> list[str]:
    ...
    if base_mode == "strict-read-only":
        allowed = set(STRICT_READ_ONLY_TOOLS)
    else:
