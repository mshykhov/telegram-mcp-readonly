...
@mcp.tool(
    annotations=ToolAnnotations(
        title="Download Media (Telegram Read-only)",
        openWorldHint=True,
...
@validate_id("chat_id")
async def download_media_readonly(
    chat_id: Union[int, str],
    message_id: int,
    account: str = None,
) -> str:
    """Download message media into one operator-controlled local directory.

    Unlike ``download_media``, this strict-mode variant accepts no destination
    path from the MCP client. It never calls a mutating Telegram RPC and can
    only create a uniquely named file below TELEGRAM_READONLY_DOWNLOAD_DIR.
    """
    try:
        raw_root = os.getenv("TELEGRAM_READONLY_DOWNLOAD_DIR", "").strip()
        if not raw_root:
            return (
                "download_media_readonly is disabled: "
                "TELEGRAM_READONLY_DOWNLOAD_DIR is not configured."
            )
        ...
        generated_name = f"telegram_{chat_digest}_{message_id}_{time.time_ns()}"
        requested_path = (root / generated_name).resolve(strict=False)
        if not _path_is_within_any_root(requested_path, [root]):
            return "Download failed: generated path is outside the configured directory."

        downloaded = await cl.download_media(msg, file=str(requested_path))
        if not downloaded:
            return f"Download failed for message {message_id}."

        final_path = Path(downloaded).resolve(strict=True)
        if not _path_is_within_any_root(final_path, [root]):
            return "Download failed: resulting path is outside the configured directory."
        try:
