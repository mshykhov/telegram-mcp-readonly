...
async def get_chats(
    ...
    Args:
        ...
        folder_id: Optional Telegram folder ID from list_folders. When set,
            only dialogs currently visible in that folder are returned.
    ...
    try:
        ...
        dialogs = await cl.get_dialogs(folder=folder_id)
...
async def list_chats(
    ...
    Args:
        ...
        with_about: If True, fetch each chat's description/bio via an additional
            ...
            disambiguation).
        folder_id: Optional Telegram folder ID from list_folders. Applies the
            complete server-side Telegram folder filter, including dynamic
            rules such as groups, contacts, muted, and archived exclusions.
    ...
    **Performance:** when `with_about=True`, makes one extra API call per chat
    ...
    try:
        cl = get_client(account)
        await ensure_connected(cl)
        if folder_id is not None and archived is not None:
            return "Use either folder_id or archived, not both."
        dialogs = await cl.get_dialogs(limit=limit, archived=archived, folder=folder_id)
