...
async def search_global(
    ...
    """
    Search for messages across the account by text content.

    Args:
        query: Telegram full-text search query.
        page: One-indexed result page.
        page_size: Results per page (1-100).
        folder_id: Optional Telegram folder ID from list_folders. Telegram
            applies the complete folder filter server-side.
    ...
    try:
        ...
        if page * page_size > 1000:
            return (
                "This request is too deep. Refine the query or request a page within 1000 results."
            )
        ...
        if folder_id is None:
            ...
            # Telethon's high-level global-search helper does not expose
            # messages.searchGlobal.folder_id, so page the raw request using
            # the same offset tuple Telegram returns to official clients.
            ...
            while len(collected) < target_end:
                ...
                entities = {
                    ...
                    for entity in list(getattr(result, "users", []))
                    + list(getattr(result, "chats", []))
                }
                batch = []
                for message in getattr(result, "messages", []):
                    if isinstance(message, types.MessageEmpty):
                        continue
                    message._finish_init(cl, entities, None)
                ...
                last = batch[-1]
                next_peer = getattr(last, "input_chat", None) or types.InputPeerEmpty()
                next_offset = (
                    getattr(result, "next_rate", 0),
                    repr(next_peer),
                    last.id,
                )
                ...
                remaining = target_end - len(collected)
                if remaining <= 0:
                    break
                request.offset_rate = next_offset[0]
                request.offset_peer = next_peer
                request.offset_id = last.id
                request.limit = min(100, remaining)

            start = (page - 1) * page_size
            messages = collected[start:target_end]
